...
export default class LevelsHelper {
  ...
  public static getLevelNumberFromString(str: string): number {
    ...
    const firstPartOfStr = strToArray[0];
    if (Number(firstPartOfStr) !== parseInt(str)) {
      throw new Error("you need to pass a string with first char number");
    }

    return parseInt(firstPartOfStr);
  }
  ...
  public static highlightLevel(
    lvl: number,
    levelsItem: NodeListOf<HTMLLIElement>
  ): void {
    levelsItem.forEach((item) =>
